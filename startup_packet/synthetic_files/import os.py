...
# Function to extract data from PDF documents
def process_pdf_file(filepath, report_id):
    all_data = []
    ...
    for page_num in range(len(doc)):
        ...
        # Extract tables using page.get_text("dict") if needed for tabular structure
        # You may need a custom logic here if the PDF has specific structures like tables
        table_data = text.splitlines()  # Basic text splitting; modify as per your PDF format
        df = pd.DataFrame([line.split() for line in table_data])  # Adjust for table structure
...
def process_files(input_directory, output_file):
    ...
    # Iterate through all files in the directory and process Excel, Word, and PDF files
    for filename in os.listdir(input_directory):
        ...
        # Process PDF files
            ...
            pdf_data = process_pdf_file(filepath, report_id)
...
output_file = r'S:\\Hack\\startup_packet\\startup_packet\\synthetic_files\\first.xlsx'
